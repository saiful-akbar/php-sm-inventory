...
if ( $data['filter']['no_pembelian'] != '0' ) {
	$no_transaksi = $data['filter']['no_pembelian'];	
}
else {
	$no_transaksi = '';	
}
...
if ( empty($data['laporan_pembelian']) ) {
	$disabled = 'disabled';
}
else {
	$disabled = '';
			...
			<div class="card-content">
				<div class="input-field">
					...
					<select name="laporan" id="laporan">
						<option value="<?= BASEURL; ?>/Laporan_pembelian" selected>Laporan Pembelian</option>
	...
	<div id="man" class="col s12">
		<div class="card material-table">
			<div class="table-header">
				<span class="table-title">Data Laporan Pembelian</span>
				<div class="actions">
			...
			<table class="datatable" width="100%">
				...
				<tbody>
					<?php foreach ($data['laporan_pembelian'] as $index => $lap_pembelian): ?>
						<tr>
							<td><?= $index + 1; ?></td>
							<td><?= $lap_pembelian['tanggal']; ?></td>
							<td><?= $lap_pembelian['nama']; ?></td>
							<td><?= $lap_pembelian['no_pembelian']; ?></td>
							<td><?= $lap_pembelian['item']; ?></td>
							<td><?= $lap_pembelian['deskripsi']; ?></td>
							<td>Rp. <?= number_format($lap_pembelian['harga_beli']); ?></td>
							<td class="center-align"><?= number_format($lap_pembelian['qty']); ?></td>
							<td>Rp. <?= number_format($lap_pembelian['subtotal']); ?></td>
						</tr>
					<?php endforeach ?>
				</tbody>
				<tfoot>
...
<div id="modal-laporan" class="modal modal-fixed-footer">	
	<form id="form-filter-laporan" action="<?= BASEURL; ?>/Laporan_pembelian" autocomplete="off">
		<div class="modal-content">
			...
			<div class="row">
				<div class="input-field col s12">
					...
					<label class="" for="no_transaksi">No Pembelian</label>
				</div>
			</div>
			<div class="row">
				<div class="input-field col s12">
