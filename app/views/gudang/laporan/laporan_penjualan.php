...
if ( $data['filter']['no_penjualan'] != '0' ) {
	$no_transaksi = $data['filter']['no_penjualan'];	
}
else {
	$no_transaksi = '';	
}
...
if ( empty($data['laporan_penjualan']) ) {
	$disabled = 'disabled';
}
else {
	$disabled = '';
			...
			<div class="card-content">
				<div class="input-field">
					...
					<select name="laporan" id="laporan">
						...
						<option value="<?= BASEURL; ?>/Laporan_penjualan" selected>Laporan Penjualan</option>
	...
	<div id="man" class="col s12">
		<div class="card material-table">
			<div class="table-header">
				<span class="table-title">Data Laporan Penjualan</span>
				<div class="actions">
			...
			<table class="datatable" width="100%">
				...
				<tbody>
					<?php foreach ($data['laporan_penjualan'] as $index => $lap_penjualan): ?>
						<tr>
							<td><?= $index + 1; ?></td>
							<td><?= $lap_penjualan['tanggal']; ?></td>
							<td><?= $lap_penjualan['no_penjualan']; ?></td>
							<td><?= $lap_penjualan['item']; ?></td>
							<td><?= $lap_penjualan['deskripsi']; ?></td>
							<td>Rp. <?= number_format($lap_penjualan['harga_jual']); ?></td>
							<td class="center-align"><?= number_format($lap_penjualan['qty']); ?></td>
							<td>Rp. <?= number_format($lap_penjualan['subtotal']); ?></td>
						</tr>
					<?php endforeach ?>
				</tbody>
				<tfoot>
...
<div id="modal-laporan" class="modal modal-fixed-footer">	
	<form id="form-filter-laporan" action="<?= BASEURL; ?>/Laporan_penjualan" autocomplete="off">
		<div class="modal-content">
			...
			<div class="row">
				<div class="input-field col s12">
					...
					<label class="" for="no_transaksi">No Penjualan</label>
				</div>
			</div>
			<div class="row">
				<div class="input-field col s12">
