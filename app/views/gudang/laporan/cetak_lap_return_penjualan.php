...
$mpdf->SetTitle('Cetak - Laporan Return Penjualan');
		...
		<table class="table" cellpadding="5" cellspacing="0" border="1">
			<thead>
				<tr class="thead">
					...
					<td align="center">No. Return Penjualan</td>
...
$i = 1;
foreach ($data['laporan_return_penjualan'] as $rpm) {
	$html .= '
		<tr class"tbody">
			<td align="center">'. $i++ .'</td>
			<td>'. $rpm["tanggal"] .'</td>
			<td>'. $rpm["keterangan"] .'</td>
			<td>'. $rpm["no_return_penjualan"] .'</td>
			<td>'. $rpm["item"] .'</td>
			<td>'. $rpm["deskripsi"] .'</td>
			<td>Rp. '. number_format($rpm["harga_jual"]) .'</td>
...
$mpdf->Output('laporan-return-penjualan.pdf', \Mpdf\Output\Destination::INLINE);
